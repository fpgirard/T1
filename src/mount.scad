...
module cutout(model,tuya,d1,d2,d3) { // cutouts for the inner valve type
     ...
     bolt_offset = tuya ? -47.5 : -46;    // Tuya bolt height, Aqara bolt height
     ...
     translate([8,-54.5,25]) rotate([0,-90,90]){ linear_extrude(.75) text(model, size=8, valign="top", halign="center",direction="ltr"); } // demarcation   
}
module base() {
  difference() {
     union() { 
         cylinder(d=od,h=oh);  // main body
         translate([0,-25,13]) rotate([90,0,0]) prismoid(size1=[35,oh], size2=[40,40], h=30, rounding=3);  
     }
     ...
     cutout(model="A3", tuya=false, d1=32.7, d2=32.7, d3=d2); translate([0,0,8]) rotate([0,0,90]) linear_sweep(octagon(id = wrench), h = 20); // Aqara with WATTS (W) LFFBV-3C-M1
     //cutout(model="T3", tuya=true, d1=32.7, d2=32.7, d3=d2); translate([0,0,8]) rotate([0,0,90]) linear_sweep(octagon(id = wrench), h = 20); // Tuya with WATTS (W) LFFBV-3C-M1
     ...
     translate([-aqara_w/2,-55,-10]) cube([aqara_w,aqara_d,aqara_h+20]); // controller slot
  }    
}
module washer() {
   difference() { 
